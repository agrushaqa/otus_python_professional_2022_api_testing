...
from src.schema import ClientsInterestsRequest, MethodRequest, OnlineScoreRequest
from src.scoring import get_interests, get_score
from src.validators.validator_birthday import ValidatorBirthday
from src.validators.validator_client_ids import ValidatorClientIds
from src.validators.validator_date import ValidatorDate
from src.validators.validator_email import ValidatorEmail
from src.validators.validator_first_name import ValidatorFirstName
from src.validators.validator_gender import ValidatorGender
from src.validators.validator_last_name import ValidatorLastName
from src.validators.validator_phone import ValidatorPhone
...
class SelectValidator:
    ...
    @staticmethod
    def separate_params_by_group(schema):  # for OnlineScoreRequest
        is_group = {}
    ...
    def _validator_nullable_params(schema, request):
        logger.info("start _validator_nullable_params")
        for i_param in schema.list_attr:
    ...
    def _validator_required_params(schema, request):
        for i_param in schema.list_attr:
            if "required" in schema.list_attr[i_param].keys():
                logger.info(f"'{i_param}' has required parameter")
                if schema.list_attr[i_param]['required'] is True:
                    logger.info(f"parameter is required for '{i_param}'")
    ...
    def online_score(self):
        ...
        if self.request['body']["login"] == config['admin']['login']:
            score = config['admin']['score']
        else:
            logger.info("user is not admin")
            with RedisStore() as redis_handler:
                score = get_score(redis_handler, phone, email, birthday,
    ...
    def clients_interests(self):
        ...
        logger.info(list_client_ids)
        dict_response = {}
        for i_client in list_client_ids:
            with RedisStore() as redis_handler:
                dict_response[i_client] = get_interests(redis_handler,
                                                        i_client)
